...
class CelCashParcelaAvulsaJob implements ShouldQueue, ShouldBeUnique
{
    ...
    {
        return 'celcash_parcela_avulsa_' . $this->id;
    }
    ...
    {
        try {
            Log::info("Iniciando processamento CelCash para parcela ID: {$this->id}");
            
            $return = CelCash::CelCashParcelaAvulsa($this->id);
            
            Log::info("CelCash processado com sucesso para parcela ID: {$this->id}", [
                'result' => $return
            ]);
            
        } catch (\Exception $e) {
            // Log do erro
            Log::error("Erro no processamento CelCash para parcela ID: {$this->id}", [
                'error' => $e->getMessage(),
