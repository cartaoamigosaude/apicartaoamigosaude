...
use App\Models\ClinicaBeneficiario;
...
class CelCashBoletoService
{
    ...
    public static function CelCashParcelasAvulsa($contrato_id)
    {
        ...
        $parcelas = Parcela::where('contrato_id', '=', $contrato_id)
            ->where('galaxPayId', '=', 0)
            ->orderBy('nparcela')
            ->get();
    ...
    public static function celcashPagamentoEmpresa($galaxId = 1)
    {
        ...
        foreach ($pagamentos as $pagamento) {
            if (Beneficiario::where('contrato_id', '=', $pagamento->id)->where('ativo', '=', 1)->count() > 0) {
                $galaxPayId = $pagamento->galaxPayId;
                $query = 'startAt=0&limit=100&galaxPayIds='.$galaxPayId;
                $consulta = CelCash::GetCharges($query, $galaxId);

                if ((isset($consulta->Charges)) and (is_array($consulta->Charges)) and (count($consulta->Charges) > 0)) {
                    foreach ($consulta->Charges as $charge) {
                        if (! is_object($charge) || ! isset($charge->Transactions) || ! is_array($charge->Transactions)) {
                            continue;
                        }

                        foreach ($charge->Transactions as $transaction) {
                            if (! is_object($transaction) || ! isset($transaction->paydayDate, $transaction->chargeMyId)) {
                                continue;
                            }

                            if (substr_count((string) $transaction->chargeMyId, '#') !== 2) {
                                continue;
                            }

                            [$contrato_id,$parcela_id] = explode('#', (string) $transaction->chargeMyId, 3);
                            if ((int) $contrato_id !== (int) $pagamento->id) {
                                continue;
                            }

                            $parcela = Parcela::find((int) $parcela_id);
                            if (! isset($parcela->id)) {
                                continue;
                            }

                            if ((int) $parcela->galaxPayId !== (int) $transaction->galaxPayId) {
                                continue;
                            }
                            ...
                            $parcela->data_pagamento = substr((string) $transaction->paydayDate, 0, 10);
                            if ($parcela->save()) {
                                $consultas[] = (string) $transaction->chargeMyId;
                            }
                        }
                    }
                }
    ...
    public static function celcashPagamentoConsulta($galaxId = 2)
    {
        ...
        $asituacao = Asituacao::find(5);

        if (isset($asituacao->id)) {
            $observacao5 = str_replace(['<p>', '</p>'], '', $asituacao->orientacao);
            $whatsapp5 = $asituacao->whatsapp;
            $whatsappc5 = $asituacao->whatsappc;
        }

        $asituacao = Asituacao::find(6);

        if (isset($asituacao->id)) {
            $observacao6 = str_replace(['<p>', '</p>'], '', $asituacao->orientacao);
            ...
            if ((isset($consulta->Charges)) and (isset($consulta->Charges[0]))) {
                ...
                if ((isset($Charge->Transactions)) and (isset($Charge->Transactions[0]))) {
                    ...
                    if (isset($Transactions->paydayDate)) {
                        $agendamento = ClinicaBeneficiario::with('clinica')->find($pagamento->id);
                        if (isset($agendamento->id)) {
                            ...
                            if ($agendamento->save()) {
                                if (($whatsapp6 != '') or ($whatsappc6 != '')) {
                                    $beneficiario = Beneficiario::with('cliente')->find($agendamento->beneficiario_id);
